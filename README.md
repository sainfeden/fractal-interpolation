...
Boxiness - the algorithm repeats small pixel structures, creating boxy artefacts. Decreasing the scale of entropy cells helps to avoid that, but it stops the IFS from sensing larger-scale structures too.
Cost - As the algorithm processes each cell individually, it does not scale too well. If larger datasets would be used, this proof-of-concept method has to be optimized. Decreasing the cell size makes the computation faster. 
